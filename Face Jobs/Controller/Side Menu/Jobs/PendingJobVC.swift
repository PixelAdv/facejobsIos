...
extension PendingJobVC{
    func GetPendingJob(page: Int){
        AF.request(APIRouter.GetpendingJobRouter(lang: MOLHLanguage.currentAppleLanguage(), Page: page, Auth: Share.savedToken as! String))
...
extension PendingJobVC{
    ...
    @objc func resendEmailPendingJob(_ sender:UIButton){
        if let id = listPending?[sender.tag].JobId{
            AF.request(APIRouter.ResendConfirmationMail(lang: MOLHLanguage.currentAppleLanguage(), JobId: id, Auth: Share.savedToken as! String))
                .responseJSON(completionHandler: { (res) in
                    print(res)
                })
                .responseDecodable(completionHandler: { [unowned self] (response: DataResponse<DeleteJopCodable, AFError>) in
