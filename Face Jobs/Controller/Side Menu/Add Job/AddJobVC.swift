...
import UIKit
import Alamofire
class AddJobVC: MainViewController {
    ...
    @IBOutlet weak var catigorystack: UIStackView!
...
//        title = "addNewJob".localized
    ...
    private func setCornersToViews()
    {
        DescriptionText.layer.cornerRadius = 4
        DescriptionText.layer.masksToBounds = true
        catigorystack.layer.cornerRadius = 4
        catigorystack.layer.masksToBounds = true
    }
    ...
    @IBAction func NextPressed(_ sender: Any) {
        ...
        let vc = Share.storyBoard.instantiateViewController(withIdentifier: "AddJobPage2") as! AddJob2VC
//        let addJopVC = vc.viewControllers.first as! AddJob2VC
        vc.JopModel = JopModel
...
extension AddJobVC{
    func GetDataToAddJob(){
        ...
        showProgress()
        AF.request(APIRouter.GetDataToAddJobRouter(lang: Share.getCurrentLanguage(), Auth: Share.savedToken as! String))
...
//                        Share.Job_Image.append((downloadImage as? UIImage)!)
                    }
                else
                    {
