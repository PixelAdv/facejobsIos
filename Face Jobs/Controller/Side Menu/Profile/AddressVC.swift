...
import UIKit
import Alamofire
class AddressVC: MainViewController {
    ...
    {
        userType == 1 ? GetEmployeeAddress() : GetClientAddress()
    }
    private func setUpView()
    {
        ...
        AddressArText.textAlignment = .right
        }
    ...
    @IBAction func SavePressed(_ sender: Any) {
        ...
        userType == 1 ? (AddEmployeeAddress(TextAr: AddressArText.text ?? "", TextEn: AddressEnText.text ?? "")) :  (AddClientAddress(TextAr: AddressArText.text ?? "", TextEn: AddressEnText.text ?? ""))
    }
}
extension AddressVC{
...
//                            self.setupPlaceholder(Message: self.AddressArText, MessageString: "Your address here...........")
                            ...
                            self.AddressArText.text = "typeYourAddress".localized
                        }else{
...
//                            self.setupPlaceholder(Message: self.AddressEnText, MessageString: "Your address here...........")
                            ...
                            self.AddressEnText.text = "typeYourAddress".localized
                        }else{
    ...
    func AddClientAddress(TextAr: String, TextEn: String){
        showProgress()
        AF.request(APIRouter.EditAddressRouter(AddressAr: TextAr, AddressEn: TextEn, Auth: Share.savedToken as! String))
            .responseDecodable(completionHandler: {  (response: DataResponse<MessageCode, AFError>) in
                self.dismisProgress()
                switch response.result {
                    case .failure(let err):
                        print(err.localizedDescription)
...
extension AddressVC: UITextFieldDelegate, UITextViewDelegate{
    ...
    @objc func didtapview(gesture: UITapGestureRecognizer) {
     ...
     func textViewShouldBeginEditing(_ textView: UITextView) -> Bool {
        if textView.text == "typeYourAddress".localized{
...
extension AddressVC {
    private func GetEmployeeAddress(){
        showProgress()
        AF.request(APIRouter.GetEmployeeAddressRouter(Auth: Share.savedToken as! String))
            .responseDecodable(completionHandler: { [unowned self] (response: DataResponse<AddressCodable, AFError>) in
                self.dismisProgress()
                switch response.result {
                    case .failure(let err):
...
//                            self.setupPlaceholder(Message: self.AddressEnText, MessageString: "Your address here...........")
    ...
    func AddEmployeeAddress(TextAr: String, TextEn: String){
        showProgress()
        AF.request(APIRouter.EditEmployeeAddressRouter(AddressAr: TextAr, AddressEn: TextEn, Auth: Share.savedToken as! String))
            .responseDecodable(completionHandler: {  (response: DataResponse<MessageCode, AFError>) in
                self.dismisProgress()
                switch response.result {
                    case .failure(let err):
